...
const Notice = ({ navigation }) => {
  const { notifications, notificationCount, loading, error, markAsRead } =
    useContext(NotificationContext);

  const handleNotificationClick = async (notification) => {
    console.log('Notification clicked:', notification);
    if (notification.status === 'Chưa đọc' && notification.backendId) {
      await markAsRead(notification.backendId);
    }
    if (notification.bookingId) {
      try {
        const bookings = await getMyBookings(navigation); // Truyền navigation
        const booking = bookings.find((b) => b.id === notification.bookingId);
        // if (booking) {
        //   navigation.navigate('BookingDetails', { booking });
        // } else {
